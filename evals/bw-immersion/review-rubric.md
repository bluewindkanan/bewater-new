...
| Explore | Before all grounding anchors are collected or explicitly Unknown, the assistant stays open-ended and offers no recommended or structured option (ground before framing). Every question earns its place: it asks only what materially changes a Charter field, and only what the user — not a public source — can answer; a publicly checkable fact (tool capability or specs, market price, technology state) is recorded as Believed or an explicit Unknown for Discover, never asked of the user. It uses mirroring or a clearly-labelled sensemaking observation when that helps the user understand the emerging situation. |
...
the user to confirm a publicly checkable fact, asks for full-document or save approval, chooses a
Converge option for the user, omits the recommendation's decision logic, or persists a draft after a
failed L0/L1 check.
