...
def test_link_mode_creates_managed_symlinks(tmp_dest):
    ...
    skill_md = skills_dest / "bw-immersion" / "SKILL.md"
    assert skill_md.is_symlink()
    assert (skills_dest / "bw-immersion" / ".bewater-managed").is_file()  # real marker, not link
...
def test_link_repair_broken_content_symlink(tmp_dest):
    ...
    broken = tmp_dest / ".claude" / "skills" / "bw-immersion" / "SKILL.md"
    broken.unlink()
...
def test_uninstall_removes_only_managed(tmp_dest):
    ...
    state_sentinels = {
        tmp_dest / "_bewater" / "config.yaml": b"invalid config sentinel\n",
        tmp_dest / "_bewater" / "ledger.yaml": b"ledger sentinel\n",
        tmp_dest / "_bewater" / "conditions.yaml": b"conditions sentinel\n",
        tmp_dest / "_bewater" / "records" / "custom.bin": b"record sentinel\x00\xff",
        tmp_dest / "_bewater-output" / "custom.bin": b"output sentinel\x00\xff",
    }
    for path, content in state_sentinels.items():
        path.write_bytes(content)

    r = _run(tmp_dest, "--uninstall")

    assert r.returncode == 0, r.stderr
    assert not (skills_dest / "bw-immersion").exists()
