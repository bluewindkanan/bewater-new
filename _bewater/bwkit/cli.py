...
def build_parser() -> argparse.ArgumentParser:
    ...
    migrate = sub.add_parser("migrate", help="explicit project-state migrations")
    migrate_sub = migrate.add_subparsers(dest="migrate_cmd", required=True)
    output_layout = migrate_sub.add_parser("output-layout", help="move workflow documents to artifacts/")
    output_layout.add_argument("root")
...
def main(argv=None, *, _stdin=None) -> int:
    ...
    if args.cmd == "init":
        ...
        return 0
    if args.cmd == "migrate" and args.migrate_cmd == "output-layout":
        from . import output_layout_migration

        try:
            result = output_layout_migration.migrate_output_layout(args.root, apply=args.apply)
        except (output_layout_migration.OutputLayoutMigrationError, OSError) as exc:
            print(f"error: {exc}", file=sys.stderr)
            return 1
        print(
            f"eligible={len(result['eligible'])} moved={result.get('moved', 0)} "
