#!/usr/bin/env python3
"""Deterministic L0 validation for a staged Charter and its staged ledger revision."""
...
def _load_ledger(path: Path) -> tuple[dict[str, Any] | None, list[str]]:
    try:
        data = yaml.safe_load(path.read_text(encoding="utf-8"))
    except FileNotFoundError:
        return None, [f"Ledger file does not exist: {path}"]
    except (OSError, yaml.YAMLError) as exc:
        return None, [f"Ledger cannot be parsed: {exc}"]
    if not isinstance(data, dict) or not isinstance(data.get("assumptions"), dict):
        return None, ["Ledger must contain an assumptions mapping."]
    return data, []


def _active_root_assumptions(ledger: dict[str, Any], branch_id: str) -> dict[str, dict[str, Any]]:
    return {
        assumption_id: record
        for assumption_id, record in ledger["assumptions"].items()
        if isinstance(record, dict)
        and record.get("branch_id") == branch_id
        and record.get("layer") == "root"
        and record.get("status") == "active"
    }


def _validate_root_assumptions(
    body: str,
    ledger: dict[str, Any],
    branch_id: str,
    artifact_id: str,
    ...
    active = _active_root_assumptions(ledger, branch_id)
    errors: list[str] = []
    if not 3 <= len(active) <= 5:
        errors.append(f"Ledger must contain 3–5 active root assumptions for {branch_id}; found {len(active)}.")
    for assumption_id, record in active.items():
        for field, expected in {
            ...
            if record.get(field) != expected:
                errors.append(f"Root assumption {assumption_id} {field} must be {expected}.")
        if not _is_text(record.get("statement")):
            errors.append(f"Root assumption {assumption_id} statement must be non-empty.")
        if not isinstance(record.get("record_revision"), int) or record["record_revision"] < 1:
            errors.append(f"Root assumption {assumption_id} record_revision must be a positive integer.")
        expected_source = f"artifact:{artifact_id}@{artifact_revision}"
        if expected_source not in record.get("derived_from", []):
            errors.append(
                f"Root assumption {assumption_id} derived_from must include {expected_source}."
            )

    table = _table(_section(body, "#### Root assumption research map"))
    if table is None:
        return errors + ["Root assumption research map must contain a Markdown table."]
    header, rows = table
    normalized = [column.strip().lower() for column in header]
    required = {"assumption", "evidence needed", "disconfirming signal"}
    if not required <= set(normalized):
        return errors + ["Root assumption research map must include Assumption, Evidence needed, and Disconfirming signal columns."]
    indices = {name: normalized.index(name) for name in required}
    mapped = {row[indices["assumption"]]: row for row in rows}
    for assumption_id in active:
        row = mapped.get(assumption_id)
        if row is None:
            errors.append(f"Root assumption research map is missing {assumption_id}.")
            continue
        for name in ("evidence needed", "disconfirming signal"):
            value = row[indices[name]]
            if not _is_text(value) or _has_placeholder(value):
                errors.append(f"Root assumption {assumption_id} {name} must be non-empty and observable.")
    return errors


def validate_files(artifact_file: Path, ledger_file: Path) -> list[str]:
    ...
    ledger, ledger_errors = _load_ledger(ledger_file)
    errors.extend(ledger_errors)
    if frontmatter is None or ledger is None:
        return errors
    ...
    errors.extend(
        _validate_root_assumptions(
            body,
            ledger,
            str(frontmatter.get("branch_id", "")),
            str(frontmatter.get("artifact_id", "")),
            frontmatter.get("revision", 0),
        )
    )
    return errors
