...
- Verify the Ideate handoff: a `concept-portfolio` with 2–4 selected concepts. If
  ...
  passed, route back to **bw-ideate** instead of producing solutions. When the
  exact Idea Pool uses `recommended_cuts`, also require
  `review.status: ready` with complete current-candidate coverage. A legacy
  Portfolio whose Pool uses `shortlist.recommended` remains readable: report it
  as not reviewed under the new contract, but do not infer missing findings or
  invalidate the handoff solely because the review block is absent.
